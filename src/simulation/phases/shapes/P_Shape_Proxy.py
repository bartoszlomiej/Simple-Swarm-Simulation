...
class P_Shape_Proxy(StaticLine):
    ...
    def __distributePriority(self):
        self.priority = self.__isHigherPriority()
        
    def __isHigherClusterID(self):
        for n in self.robot.neighbors:
            if n.cluster_id < self.robot.cluster_id:
                return True
        return False

    def __higherPriorityWaits(self):
        pass

    def __lowerPrioritySelfDivide(self):
        pass

    def __createSuperSuperCluster(self):
    ...
    higher - wait (keep static line)
    lower - downgrade if not self.robot.divisions -> downgrade
            else upgrade to P-shape
    '''
